...
for i in range(len(EXPERIMENT)):
    for j in range(len(MODEL)):
        ...
        if (os.path.isfile(FILE)==False):
            ...
            ds_reg = ds.sel(lat=slice(LATMIN, LATMAX), lon=slice(LONMIN,LONMAX), time=slice(TIMEMIN, TIMEMAX),plev=LEVEL)
...
#Define figure
fig, ax = plt.subplots(figsize=(3*len(EXPERIMENT),1.3*len(MODEL)))

#Define grid for subplots
gs = gridspec.GridSpec(len(MODEL),len(EXPERIMENT),width_ratios=3*np.ones(len(EXPERIMENT)),height_ratios=1.3*np.ones(len(MODEL)))     
...
for i in range(len(EXPERIMENT)):
    for j in range(len(MODEL)):
        ...
        if np.sum(pandas.isnull(lat_exp_mod[(i,j)]))==0:
            ...
            ax = plt.subplot(gs[i+j*len(EXPERIMENT)],projection=ccrs.Robinson(central_longitude=180))
            ...
            ax.set_extent([LONMIN, 359.9, LATMIN, LATMAX], crs=crs_latlon)
...
fig.subplots_adjust(left=0.03, bottom=0.03, right=0.91,top=0.93, wspace=0.3 ,hspace=0.1)   
...
plt.figtext(1.2/len(EXPERIMENT),.97, 'Trend ' + VAR + ' ' +str(int(LEVEL/100)) + ' hPa (mgp/decade) ' + str(YEARMIN+1) + '-' + str(YEARMAX), fontsize=12)

#Save figure
fig.savefig(FIG_PATH + 'TrendDJF_' + VAR + str(int(LEVEL/100)) +'_' + str(YEARMIN+1) + '_' + str(YEARMAX)+ '.png', dpi=500, bbox_inches='tight')

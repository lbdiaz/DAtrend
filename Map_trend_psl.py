...
#Define figure
fig, ax = plt.subplots(figsize=(3*len(EXPERIMENT),1.5*len(MODEL)))
...
for i in range(len(EXPERIMENT)):
    for j in range(len(MODEL)):
        ...
        if np.sum(pandas.isnull(lat_exp_mod[(i,j)]))==0:
            ...
            im=ax.contourf(lons, lats, tend_var_MM[(i,j)]*10, levels, cmap=cmap, transform=crs_latlon, extend='both')
            ...
            ax.set_xticks(np.linspace(LONMIN, int(LONMAX+0.1),  4), crs=crs_latlon)
...
plt.figtext(1.2/len(EXPERIMENT),.97, 'Trend ' + VAR + ' *0.1hPa/decade) ' + str(YEARMIN+1) + '-' + str(YEARMAX), fontsize=12)
